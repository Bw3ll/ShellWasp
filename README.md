...
## Updates
* August 2026 update: ShellWasp’s parameter handling has been substantially expanded. The tool can now identify pointer parameters, allocate stack-backed storage for them, and build referenced structures and unions. From these, the resulting addresses can then be supplied to function parameter. SHellWasp also handles nested pointer fields, optional as well as intentionally NULL values, output parameters, and structures such as OBJECT_ATTRIBUTES or IO_STATUS_BLOCK.
