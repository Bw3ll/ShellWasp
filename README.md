...
ShellWasp supports Windows 7, 10, and 11. Users can choose which releases to target rather than supporting every possible build. That helps keep the resulting shellcode smaller and easier to manage. ShellWasp also handles the fact that syscall invocation differs between Windows 7 and Windows 10/11. To achieve a more compact shellcode size, ShellWasp utilizes precomputed syscall tables in JSON format. This allows us to keep the shellcode size minimal. 
